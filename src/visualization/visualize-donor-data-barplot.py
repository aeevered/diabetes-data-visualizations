...
user_stats_df["Years Living With Category"] = user_stats_df["Years Living With"].apply(
    get_ylw_category
)
user_stats_df["Age and Years Living With Category"] = (
    user_stats_df["Age Category"] * 10 + user_stats_df["Years Living With Category"]
)

# Add Labels
user_stats_df["Age"] = user_stats_df["Age Category"].apply(get_age_label)
user_stats_df["dAge (years since diagnosis)"] = user_stats_df[
    "Years Living With Category"
].apply(get_ylw_label)
user_stats_df = user_stats_df.sort_values(
    by=[
        "Age and Years Living With Category",
        "Years Living With Category",
        "Age Category",
    ],
    ascending=True,
...
def add_metric(initial_table, categories, field):
    ...
    metric_table = user_stats_df.groupby([categories])[field].describe(
...
age_metric_table = pd.DataFrame(
    {
        "Age Category": [1, 2, 3, 4, 5],
        "Years Living With Category": ["NaN", "NaN", "NaN", "NaN", "NaN"],
    }
)
ylw_metric_table = pd.DataFrame(
    {"Age Category": ["NaN", "NaN", "NaN"], "Years Living With Category": [1, 2, 3]}
)
age_ylw_metric_table = pd.DataFrame(
    {
        "Age and Years Living With Category": user_stats_df[
            "Age and Years Living With Category"
        ].unique(),
        "Age Category": ["NaN"]
        * len(user_stats_df["Age and Years Living With Category"].unique()),
        "Years Living With Category": ["NaN"]
        * len(user_stats_df["Age and Years Living With Category"].unique()),
    }
)
...
for metric in metrics:
    ...
    ylw_metric_table = add_metric(
        ylw_metric_table, "Years Living With Category", metric
    )
    age_ylw_metric_table = add_metric(
        age_ylw_metric_table, "Age and Years Living With Category", metric
    )
...
# Add Labels
summary_metrics_table["Age"] = summary_metrics_table["Age Category"].apply(
    get_age_label
)
summary_metrics_table["dAge (years since diagnosis)"] = summary_metrics_table[
    "Years Living With Category"
].apply(get_ylw_label)
summary_metrics_table["Age & dAge"] = summary_metrics_table[
    "Age and Years Living With Category"
].apply(get_ylw_age_label)
...
summary_metrics_table["Glucose Management Index.count"] = (
    summary_metrics_table["Glucose Management Index.count"]
    .apply(place_value)
    .astype(str)
...
summary_metrics_table["Age"] = (
    "<b>"
    + summary_metrics_table["Age"]
    + " </b><br>(n="
    + summary_metrics_table["Glucose Management Index.count"].astype(str)
    + ")"
)
summary_metrics_table["dAge (years since diagnosis)"] = (
    "<b>"
    + summary_metrics_table["dAge (years since diagnosis)"]
    + "   </b><br>(n="
    + summary_metrics_table["Glucose Management Index.count"].astype(str)
...
summary_metrics_table["Age & dAge"] = (
    "<b>"
    + summary_metrics_table["Age & dAge"]
    + "   </b>    (n="
    + summary_metrics_table["Glucose Management Index.count"].astype(str)
...
# Metrics
y_metrics = ["Age", "dAge (years since diagnosis)", "Age & dAge"]
...
def create_25(metric):
    bar_trace = go.Bar(
        ...
        name="80% of Data",
        visible=get_visibility(metric),
        x=summary_metrics_table[metric + ".25%"]
        - summary_metrics_table[metric + ".10%"],
        y=summary_metrics_table[y_starting_metric],
        hoverinfo="skip",
        orientation="h",
        width=starting_width,
        textposition="inside",
        marker=dict(color=color_10_to_90),
    )
    bar_trace.yaxis = "y"
    bar_trace.xaxis = "x"
    return bar_trace
...
def create_75(metric):
    bar_trace = go.Bar(
        ...
        showlegend=True,
        visible=get_visibility(metric),
        x=summary_metrics_table[metric + ".75%"]
        - summary_metrics_table[metric + ".25%"],
        y=summary_metrics_table[y_starting_metric],
        hoverinfo="skip",
        orientation="h",
        width=starting_width,
        textposition="inside",
        marker=dict(color=color_iqr),
    )
    bar_trace.yaxis = "y"
    bar_trace.xaxis = "x"
    return bar_trace
...
def create_90(metric):
    bar_trace = go.Bar(
        ...
        showlegend=False,
        visible=get_visibility(metric),
        x=summary_metrics_table[metric + ".90%"]
        - summary_metrics_table[metric + ".75%"],
        y=summary_metrics_table[y_starting_metric],
        hoverinfo="skip",
        orientation="h",
        width=starting_width,
...
def get_x_axis_attributes(metric):
    attributes = dict(
        rangemode="tozero",
        range=x_axis_range.loc[x_axis_range["metric"] == metric, "x_axis_range"].iloc[
            0
        ],
        ...
        domain=[0, 1],
        zeroline=True,
        title=x_axis_label.loc[x_axis_label["metric"] == metric, "x_axis_label"].iloc[
            0
        ],
        showline=False,
...
def create_y_button(metric, marker_size, width):
    ...
    button = dict(
        args=[
            {
                "y": [summary_metrics_table[metric]],
...
layout = go.Layout(
    ...
    legend=go.layout.Legend(
        ...
        y=0.8,
        traceorder="reversed",
        font=dict(family="Raleway", size=12, color="black"),
        bgcolor="white",
        bordercolor="white",
        borderwidth=2,
    ),
    ...
    yaxis=get_y_axis_attributes(0, 0.8),
    yaxis2=get_y_axis_attributes(0, 0.8),
)
...
for index in range(0, len(y_metrics)):
    y_buttons.append(
        create_y_button(y_metrics[index], median_dot_sizes[index], width[index])
    )
